...
interface HistoryItem {
  ...
  voiceType: VoiceType;
  text: string;
  audioUrl?: string; // Optional if we just want to show text for now
  transcript?: string;
  createdAt: string;
}
...
export default function Home() {
  ...
  // Simple mock function to simulate API call since backend is not implemented yet
  const handleGenerate = async (e: FormEvent) => {
    ...
    try {
      ...
      const newItem: HistoryItem = {
        ...
        voiceType,
        text: inputText,
        // Mock audio URL (using a placeholder or empty for now as we don't have the backend)
        audioUrl: "",
        transcript: voiceType === "dog" ? "Woof! " + inputText : "Squawk! " + inputText,
      ...
      <main className="max-w-2xl mx-auto space-y-8">
        ...
        <section className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <button
            ...
            className={`p-6 rounded-2xl border-2 text-left transition-all duration-200 ${voiceType === "dog"
          ...
          <button
            ...
            className={`p-6 rounded-2xl border-2 text-left transition-all duration-200 ${voiceType === "parrot"
          ...
          <form onSubmit={handleGenerate} className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex gap-2">
                <button
                  type="button"
                  onClick={() => handleInsertExample("Tell me a fun fact about space.")}
        ...
        <section className="space-y-6">
          ...
          <div className="space-y-4">
            {history.length === 0 ? (
              ...
              history.map((item) => (
                <div key={item.id} className="bg-white dark:bg-zinc-900 rounded-2xl p-6 shadow-sm border border-zinc-200 dark:border-zinc-800 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="space-y-3 pt-2">
                    {item.audioUrl && (
                      <audio controls className="w-full h-10" src={item.audioUrl}>
        ...
        <footer className="pt-12 pb-8 border-t border-zinc-200 dark:border-zinc-800">
          <div className="text-center md:text-left space-y-4 max-w-lg">
            ...
            <p className="text-zinc-600 dark:text-zinc-400 leading-relaxed">
              Future: speaking with real animals (birds, dolphins, etc.) using bioacoustics and AI.
            </p>
